...
/// Provides interface for embedding images into Qlzb editor.
// TODO: allow configuring image sources and related toolbar buttons.
@experimental
abstract class QlzbImageDelegate<S> {
...
class QlzbImage extends StatefulWidget {
  const QlzbImage({Key key, @required this.node, @required this.delegate})
      : super(key: key);

  final EmbedNode node;
  final QlzbImageDelegate delegate;

  @override
  _QlzbImageState createState() => _QlzbImageState();
}

class _QlzbImageState extends State<QlzbImage> {
  String get imageSource {
    EmbedAttribute attribute = widget.node.style.get(QDocAttribute.embed);
