...
/// Controls instance of [QlzbEditor].
class QlzbController extends ChangeNotifier {
  QlzbController(QDocDocument document)
      : assert(document != null),
        document = document;

  /// Qlzb document managed by this controller.
  ...
  QDocStyle get toggledStyles => _toggledStyles;
  QDocStyle _toggledStyles = QDocStyle();
  ...
  void formatText(int index, int length, QDocAttribute attribute) {
    final change = document.format(index, length, attribute);
    _lastChangeSource = ChangeSource.local;

    if (length == 0 &&
        (attribute.key == QDocAttribute.bold.key ||
            attribute.key == QDocAttribute.italic.key)) {
  ...
  void formatSelection(QDocAttribute attribute) {
