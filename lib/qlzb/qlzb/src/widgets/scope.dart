...
/// Provides access to shared state of [QlzbEditor] or [QlzbView].
///
/// A scope object can be created by an editable widget like [QlzbEditor] in
...
/// [QlzbScope.editable] constructor.
///
/// If a scope object is created by a view-only widget like [QlzbView] then
/// it only provides access to [imageDelegate].
///
/// Can be retrieved using [QlzbScope.of].
class QlzbScope extends ChangeNotifier {
  /// Creates a view-only scope.
  ///
  /// Normally used in [QlzbView].
  QlzbScope.view({QlzbImageDelegate imageDelegate})
      : isEditable = false,
        _mode = QlzbMode.view,
  ...
  ///
  /// Normally used in [QlzbEditor].
  QlzbScope.editable({
    @required QlzbMode mode,
    @required QlzbController controller,
    @required FocusNode focusNode,
    @required FocusScopeNode focusScope,
    QlzbImageDelegate imageDelegate,
  })  : assert(mode != null),
        ...
        _renderContext = QlzbRenderContext() {
  ...
  static QlzbScope of(BuildContext context) {
    final widget =
        context.dependOnInheritedWidgetOfExactType<QlzbScopeAccess>();
    return widget.scope;
  }

  QlzbImageDelegate _imageDelegate;
  QlzbImageDelegate get imageDelegate => _imageDelegate;
  set imageDelegate(QlzbImageDelegate value) {
  ...
  QlzbMode _mode;
  QlzbMode get mode => _mode;
  set mode(QlzbMode value) {
    assert(value != null);
    if (_mode != value) {
      _mode = value;
      notifyListeners();
  ...
  QlzbController get controller => _controller;
  set controller(QlzbController value) {
  ...
  QlzbRenderContext _renderContext;
  QlzbRenderContext get renderContext => _renderContext;

  QDocStyle get selectionStyle => _selectionStyle;
  QDocStyle _selectionStyle;
  ...
  /// Whether this scope is backed by editable Qlzb widgets or read-only view.
  ///
  /// Returns `true` if this scope provides Qlzb interface that allows editing
  /// (e.g. created by [QlzbEditor]). Returns `false` if this scope provides
  /// read-only view (e.g. created by [QlzbView]).
  ///
  ...
  void formatSelection(QDocAttribute value) {
    assert(isEditable);
    assert(!_disposed);
  ...
  @override
  String toString() {
    return '$QlzbScope#${shortHash(this)}';
  }
}

class QlzbScopeAccess extends InheritedWidget {
  final QlzbScope scope;

  QlzbScopeAccess({Key key, @required this.scope, @required Widget child})
      : super(key: key, child: child);

  @override
  bool updateShouldNotify(QlzbScopeAccess oldWidget) {
