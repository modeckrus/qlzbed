...
class ResolveLineFormatRule extends FormatRule {
  ...
  @override
  Delta apply(Delta document, int index, int length, QDocAttribute attribute) {
    if (attribute.scope != QDocAttributeScope.line) return null;
  ...
  Delta _applyAttribute(String text, QDocAttribute attribute) {
    final result = Delta();
    var offset = 0;
    var lf = text.indexOf('\n');
    while (lf >= 0) {
...
class ResolveInlineFormatRule extends FormatRule {
  ...
  @override
  Delta apply(Delta document, int index, int length, QDocAttribute attribute) {
    if (attribute.scope != QDocAttributeScope.inline) return null;
...
class FormatLinkAtCaretPositionRule extends FormatRule {
  ...
  @override
  Delta apply(Delta document, int index, int length, QDocAttribute attribute) {
    if (attribute.key != QDocAttribute.link.key) return null;
