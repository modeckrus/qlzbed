  ...
  Delta apply(Delta document, int index, String text) {
    ...
    if (target.data.startsWith('\n')) {
      ...
      if (target.attributes != null &&
          target.attributes.containsKey(QDocAttribute.heading.key)) {
        resetStyle = QDocAttribute.heading.unset.toJson();
      }
      return Delta()
        ..retain(index)
        ..insert('\n', target.attributes)
...
class AutoExitBlockRule extends InsertRule {
  ...
  Delta apply(Delta document, int index, String text) {
    ...
    final isInBlock = target.isNotPlain &&
        target.attributes.containsKey(QDocAttribute.block.key);
    if (isEmptyLine(previous, target) && isInBlock) {
      ...
      attributes.addAll(QDocAttribute.block.unset.toJson());
...
class PreserveInlineStylesRule extends InsertRule {
  ...
  Delta apply(Delta document, int index, String text) {
    ...
    final hasLink =
        (attributes != null && attributes.containsKey(QDocAttribute.link.key));
    if (!hasLink) {
      return Delta()
        ..retain(index)
    ...
    if (!nextAttributes.containsKey(QDocAttribute.link.key)) {
...
class AutoFormatLinksRule extends InsertRule {
  ...
  Delta apply(Delta document, int index, String text) {
    ...
    try {
      ...
      if (attributes.containsKey(QDocAttribute.link.key)) return null;

      attributes
          .addAll(QDocAttribute.link.fromString(link.toString()).toJson());
      return Delta()
...
class PreserveBlockStyleOnPasteRule extends InsertRule {
  ...
  Delta apply(Delta document, int index, String text) {
    ...
    if (lineStyle != null) {
      if (lineStyle.containsKey(QDocAttribute.heading.key)) {
        resetStyle = QDocAttribute.heading.unset.toJson();
      }

      if (lineStyle.containsKey(QDocAttribute.block.key)) {
        blockStyle = <String, dynamic>{
          QDocAttribute.block.key: lineStyle[QDocAttribute.block.key]
        };
      }
    }
