...
/// A line of rich text in a QDoc document.
///
...
class LineNode extends ContainerNode<LeafNode>
  ...
  QDocStyle collectStyle(int offset, int length) {
    final local = math.min(this.length - offset, length);

    var result = QDocStyle();
    final excluded = <QDocAttribute>{};

    void _handle(QDocStyle style) {
      if (result.isEmpty) {
  ...
  @override
  void insert(int index, String text, QDocStyle style) {
  ...
  @override
  void retain(int index, int length, QDocStyle style) {
    ...
    if (isLineFormat) {
      assert(
          style.values.every((attr) => attr.scope == QDocAttributeScope.line),
      ...
      assert(index + local != thisLength,
          'It is not allowed to apply inline attributes to line itself.');
      assert(style.values
          .every((attr) => attr.scope == QDocAttributeScope.inline));
  ...
  void _formatAndOptimize(QDocStyle newStyle) {
    if (newStyle == null || newStyle.isEmpty) return;

    applyStyle(newStyle);
    if (!newStyle.contains(QDocAttribute.block)) {
      return;
    } // no block-level changes

    final blockStyle = newStyle.get(QDocAttribute.block);
    if (parent is BlockNode) {
      final parentStyle = (parent as BlockNode).style.get(QDocAttribute.block);
      if (blockStyle == QDocAttribute.block.unset) {
        unwrap();
    ...
    } else if (blockStyle != QDocAttribute.block.unset) {
  ...
  void _insertSafe(int index, String text, QDocStyle style) {
    assert(index == 0 || (index > 0 && index < length));
