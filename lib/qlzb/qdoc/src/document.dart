...
/// Represents a change in a [QDocDocument].
class QDocChange {
  QDocChange(this.before, this.change, this.source);
...
class QDocDocument {
  /// Creates new empty QDoc document.
  QDocDocument()
      : _heuristics = QDocHeuristics.fallback,
        _delta = Delta()..insert('\n') {
    _loadDocument(_delta);
  }

  QDocDocument.fromJson(List data)
      : _heuristics = QDocHeuristics.fallback,
        _delta = Delta.fromJson(data) {
    _loadDocument(_delta);
  }

  QDocDocument.fromDelta(Delta delta)
      : assert(delta != null),
        _heuristics = QDocHeuristics.fallback,
        _delta = delta {
    _loadDocument(_delta);
  ...
  final QDocHeuristics _heuristics;
  ...
  /// Stream of [QDocChange]s applied to this document.
  Stream<QDocChange> get changes => _controller.stream;

  final StreamController<QDocChange> _controller = StreamController.broadcast();
  ...
  /// produces a [QDocChange] with source set to [ChangeSource.local].
  ///
  ...
  /// unchanged and no [QDocChange] is published to [changes] stream.
  Delta format(int index, int length, QDocAttribute attribute) {
  ...
  QDocStyle collectStyle(int index, int length) {
  ...
  void compose(Delta change, ChangeSource source) {
    ...
    for (final op in change.toList()) {
      final attributes =
          op.attributes != null ? QDocStyle.fromJson(op.attributes) : null;
      if (op.isInsert) {
    ...
    _controller.add(QDocChange(before, change, source));
  }
  ...
  void _checkMutable() {
    assert(!_controller.isClosed,
        'Cannot modify QDoc document after it was closed.');
  }
